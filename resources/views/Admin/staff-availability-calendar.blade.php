                        ...
                        @if($period)
                            <div class="relative group">
                                <div class="bg-red-100 border border-red-400 rounded-lg p-2 text-xs text-red-800 cursor-pointer"
                                     wire:click="openEdit({{ $period->unavailabilityPeriodId }})">
                                    <div class="font-semibold">Unavailable</div>
                                    @if($period->description)
                                        <div class="truncate max-w-[90px]">{{ $period->description }}</div>
                                    @endif
                                </div>
                                <button wire:click="delete({{ $period->unavailabilityPeriodId }})"
        ...
        <div class="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg mb-2">
            <div>
                <span class="font-semibold text-gray-900">
                    ...
                    @if($period->startDate->format('Y-m-d') !== $period->endDate->format('Y-m-d'))
            ...
            <div class="flex gap-2">
                <flux:button wire:click="openEdit({{ $period->unavailabilityPeriodId }})" size="sm" variant="ghost" icon="pencil" />
                <flux:button wire:click="delete({{ $period->unavailabilityPeriodId }})" wire:confirm="Remove this period?" size="sm" variant="ghost" icon="trash" />
            </div>
        </div>
    @empty
